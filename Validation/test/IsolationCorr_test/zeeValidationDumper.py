...
process.flashggPreselectedDiPhotonsUncorrPho = process.flashggPreselectedDiPhotons.clone()
process.flashggPreselectedDiPhotonsUncorrPho.variables[-1] = "-(passElectronVeto - 1)"
process.flashggPreselectedDiPhotonsUncorrPho.src = cms.InputTag("flashggDiPhotons")
...
# New process for dumping variables before correction
process.diphotonDumperBeforeCorr.rho = cms.InputTag("fixedGridRhoAll")
process.diphotonDumperBeforeCorr.src = cms.InputTag("flashggPreselectedDiPhotonsUncorrPho")
# process.diphotonDumperBeforeCorr.src = cms.InputTag("flashggDiPhotons")
process.diphotonDumperBeforeCorr.dumpHistos = False
process.diphotonDumperBeforeCorr.dumpTrees  =  True
process.diphotonDumperBeforeCorr.dumpGlobalVariables = cms.untracked.bool(True)
process.diphotonDumperBeforeCorr.globalVariables = process.globalVariables

cfgTools.addCategories(process.diphotonDumperBeforeCorr,
                       ## categories definition
                       ## cuts are applied in cascade. Events getting to these categories have already failed the "Reject" selection
                       [("All","1", 0)
                        # ("EB", "abs(superCluster.eta)<1.479", 0),
...
process.diphotonDumperBeforeCorr.nameTemplate ="zeevalidation_$SQRTS_$LABEL_$SUBCAT"
...
process.p = cms.Path(process.flashggUpdatedIdMVADiPhotons*process.flashggDiPhotonSystematics*process.dataRequirements*process.flashggPreselectedDiPhotons*process.flashggDiPhotonMVA*process.DiPhotonWithZeeMVADumper*process.diphotonDumper*process.flashggPreselectedDiPhotonsUncorrPho*process.diphotonDumperBeforeCorr)
